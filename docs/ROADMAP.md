...
| `tests/` | 182 files, **4142 offline passing** (+42 skipped) | ✅ | add tests with each new module |
...
**Goal:** on a fresh machine, get all 4142 offline tests green and reproduce ≥1 live scenario.
- [ ] `uv sync` + `uv run pytest -q` → 4142 passing (+42 skipped) (offline).
...
**Acceptance:** `make test` → 4142 offline tests green; `make seed-registry` → dual-gate `ok`;
...
- **offline**: every new module gets `tests/test_*.py` (mock AWS); keep `uv run pytest -q` green (now 4142, +6 skipped, only grows).
