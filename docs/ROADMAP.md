...
| `tests/` | 184 files, **4161 offline passing** (+42 skipped) | ✅ | add tests with each new module |
...
**Goal:** on a fresh machine, get all 4161 offline tests green and reproduce ≥1 live scenario.
- [ ] `uv sync` + `uv run pytest -q` → 4161 passing (+42 skipped) (offline).
...
**Acceptance:** `make test` → 4161 offline tests green; `make seed-registry` → dual-gate `ok`;
...
- **offline**: every new module gets `tests/test_*.py` (mock AWS); keep `uv run pytest -q` green (now 4161, +6 skipped, only grows).
