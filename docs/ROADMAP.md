...
| `tests/` | 157 files, **3812 offline passing** (+6 skipped) | ✅ | add tests with each new module |
...
**Goal:** on a fresh machine, get all 3812 offline tests green and reproduce ≥1 live scenario.
- [ ] `uv sync` + `uv run pytest -q` → 3812 passing (+6 skipped) (offline).
...
**Acceptance:** `make test` → 3812 offline tests green; `make seed-registry` → dual-gate `ok`;
