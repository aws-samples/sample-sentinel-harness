...
| `tests/` | 137 files, **3025 offline passing** (+6 skipped) | ✅ | add tests with each new module |
...
**Goal:** on a fresh machine, get all 3025 offline tests green and reproduce ≥1 live scenario.
- [ ] `uv sync` + `uv run pytest -q` → 3025 passing (+6 skipped) (offline).
...
**Acceptance:** `make test` → 3025 offline tests green; `make seed-registry` → dual-gate `ok`;
...
**Acceptance:** suite 2493 → **3025** offline passing (+8 skipped), ruff clean,
...
guards, expected green in both states. Suite 2590 → **3025**; ruff clean; both
...
pre-R10 source** (verified by reverting the three modules). Suite 2649 → **3025**;
...
dedup tripwires and regression guards, green in both states. Suite 2671 → **3025**
...
happy-path and fail-closed regression guards. Suite 2730 → **3025** collected; ruff
