...
| `tests/` | 137 files, **3728 offline passing** (+6 skipped) | ✅ | add tests with each new module |
...
**Goal:** on a fresh machine, get all 3728 offline tests green and reproduce ≥1 live scenario.
- [ ] `uv sync` + `uv run pytest -q` → 3728 passing (+6 skipped) (offline).
...
**Acceptance:** `make test` → 3728 offline tests green; `make seed-registry` → dual-gate `ok`;
