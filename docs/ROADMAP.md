...
| `tests/` | 167 files, **3903 offline passing** (+6 skipped) | ✅ | add tests with each new module |
...
**Goal:** on a fresh machine, get all 3903 offline tests green and reproduce ≥1 live scenario.
- [ ] `uv sync` + `uv run pytest -q` → 3903 passing (+6 skipped) (offline).
...
**Acceptance:** `make test` → 3903 offline tests green; `make seed-registry` → dual-gate `ok`;
...
- **offline**: every new module gets `tests/test_*.py` (mock AWS); keep `uv run pytest -q` green (now 3903, +6 skipped, only grows).
