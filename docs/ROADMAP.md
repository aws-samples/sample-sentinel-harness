...
| `tests/` | 171 files, **4039 offline passing** (+6 skipped) | ✅ | add tests with each new module |
...
**Goal:** on a fresh machine, get all 4039 offline tests green and reproduce ≥1 live scenario.
- [ ] `uv sync` + `uv run pytest -q` → 4039 passing (+6 skipped) (offline).
...
**Acceptance:** `make test` → 4039 offline tests green; `make seed-registry` → dual-gate `ok`;
...
- **offline**: every new module gets `tests/test_*.py` (mock AWS); keep `uv run pytest -q` green (now 4039, +6 skipped, only grows).
