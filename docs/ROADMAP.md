...
| `tests/` | 137 files, **3706 offline passing** (+6 skipped) | ✅ | add tests with each new module |
...
**Goal:** on a fresh machine, get all 3706 offline tests green and reproduce ≥1 live scenario.
- [ ] `uv sync` + `uv run pytest -q` → 3706 passing (+6 skipped) (offline).
...
**Acceptance:** `make test` → 3706 offline tests green; `make seed-registry` → dual-gate `ok`;
