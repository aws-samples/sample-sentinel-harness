...
| `tests/` | 179 files, **4111 offline passing** (+20 skipped) | ✅ | add tests with each new module |
...
**Goal:** on a fresh machine, get all 4111 offline tests green and reproduce ≥1 live scenario.
- [ ] `uv sync` + `uv run pytest -q` → 4111 passing (+20 skipped) (offline).
...
sentinel_inference_gateway/                    # standalone LiteLLM inference gateway (single model entry + audit)
...
**Acceptance:** `make test` → 4111 offline tests green; `make seed-registry` → dual-gate `ok`;
...
- **offline**: every new module gets `tests/test_*.py` (mock AWS); keep `uv run pytest -q` green (now 4111, +6 skipped, only grows).
...
**Acceptance:** suite 2493 → **2590** offline passing (+20 skipped), ruff clean,
